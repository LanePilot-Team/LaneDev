# NavSim LaneDev — 楠梓區車道級導航模擬前端（開發版）

台灣機車友善車道級導航系統畢業專題的模擬前端（純前端、無後端）：
React 19 + TypeScript + Vite + MapLibre GL + Turf + three.js。

> 本資料夾為獨立可執行單元。**LaneDev = 開發版**（含編輯模式全功能）；
> 發布版 `../LaneNav` 由 `npm run sync-lanenav` 鏡像共用碼產生（無編輯模式，port 5181）。
> 文件分工：`說明書.md` = 元件怎麼用/核心在哪、`SDD.md` = 設計規格與復現步驟
> （皆在畢專主目錄）；本 README 只做快速上手與功能總覽。

## 快速開始

```powershell
cd nav_simulator\LaneDev
npm install
npm run dev            # http://localhost:5180（host 已開，手機走 Tailscale IP）
npm run build          # tsc -b && vite build（build 即型別檢查；無另設 lint/test）
npm run sync-lanenav   # 共用碼白名單鏡像 → ../LaneNav（先刪後複製、冪等）
```
...
| 真 GPS 導航 | `watchPosition`＋沿線投影；偏離 60m×3 次自動重規劃（10s 冷卻）；需 HTTPS（手機用 `tailscale serve --bg --https=443 http://localhost:5180`）；Wake Lock 防熄屏 |
...
src/app/mapCore.ts       地圖初始化＋共用 refs＋重繪函式（LaneDev/LaneNav 共用）
...
src/edit/                LaneDev 專屬：useEditor＋EditPanels（不進 sync 白名單）
...
正式清單維護在 `SDD.md` §10 與計畫書 B-11/B-12 backlog。摘要：
