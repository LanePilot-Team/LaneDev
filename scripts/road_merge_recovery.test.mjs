...
test('正式報告分開列出已升級與已回退並記錄來源 commit', () => {
  ...
  assert.match(markdown, /## 處置結果與判定說明/)
  assert.match(markdown, /`replayable`/)
  assert.match(markdown, /`recoverable_via_provenance`/)
  assert.match(markdown, /`needs_manual_review`/)
  assert.match(markdown, /`legacy_destructive`/)
  assert.match(markdown, /主段與次段不代表地圖上的左右順序/)
})
