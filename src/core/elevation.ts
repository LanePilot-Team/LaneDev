...
const ELEVATED_WAY_IDS = new Set([
  ...
  // 楠梓交流道機車專用道匝道高架：節點相接的連續 298m 鏈，末端接上機車專用道
  // 高架 way/25724904（共用 node 1196965025）。不列的話這段被當平面路畫在地上，
  // 旁邊主橋卻在 6m 高——畫面上整條掉在地面，與橋面分裂。
  //   103679024(65m) → 230216189(61m) → 230216191(115m) → 230213636(57m)
  // 都是 layer=1；103679024 無 bridge tag 但夾在鏈中間，一起抬才不會斷階。
  // 刻意不含 way/293380617：bridge=yes 但 layer=0，是與路面同高的 50m 小橋
  //（同仁武橋判準，見本檔開頭「bridge=yes ≠ 高架」）。
  103679024, 230216189, 230216191, 230213636,
])
