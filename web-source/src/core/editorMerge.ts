...
export function uniteEditors(
  ...
  const zoneIds = new Set(fileZones.map((zone) => String(zone.id)))
  const missingZones = (browser.waiting_zones ?? [])
    .filter((zone) => !zoneIds.has(String(zone.id)))
  ...
  const waitingZones = [...fileZones, ...missingZones]
    .filter((zone) => !tombstoned.has(String(zone.id)))

  // 刻意不做「瀏覽器比較新就取代檔案版本」的待轉區仲裁。那正是 2026-07-27
  // 一次少掉 258 筆的成因：舊分頁的 updated_at 一定是「現在」，內容卻是舊的，
  // 用時間戳判勝負只會讓舊資料蓋掉新資料。檔案永遠是完整且權威的一份。
  return {
    editor: {
    ...
    recovered: missing.length + missingZones.length,
  }
}
