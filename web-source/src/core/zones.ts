...
export function loadZones(): Zone[] {
  return staticZones().filter(z => z.intersectionId && z.from).map(z => ({ ...z, visible: z.visible !== false }))
}

export function saveZones(_zones: Zone[]) { throw new Error('App 不允許修改待轉區') }

/** 記錄使用者明確刪除的靜態匯入待轉格，避免下次啟動被自動匯入復活。 */
export function loadDeletedZoneIds(): Set<string> { return new Set(staticDeletedZoneIds()) }

export function markZoneDeleted(_id: string) { throw new Error('App 不允許刪除待轉區') }
