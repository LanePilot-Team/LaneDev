...
export function LanePreviewPanel({ model }: { model: LanePreviewModel }) {
  ...
  return (
    <div className="lane-preview" aria-label={previewLabel(model)}>
      ...
      {model.inferenceNote && <div className="lane-preview-note" aria-label={model.inferenceNote}>車道資訊為推估</div>}
      {model.warningNote && <div className="lane-preview-warning" aria-label={model.warningNote}>勿勉強變道，錯過將重算</div>}
    </div>
  )
}
